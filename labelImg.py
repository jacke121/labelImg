    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Up:
            ...
            if not items:
                return
            item = items[0]
            self.currIndex = self.superwindow.imageList.index(str(item.text()))
            # if currIndex < len(self.superwindow.imageList):
            filename = self.superwindow.imageList[self.currIndex]
            if filename:
                # item =
                # item = None
                self.removeItemWidget(self.takeItem(self.currIndex))
                test_anno = os.path.dirname(filename) + "/../test_mouse/Annotations/"
                os.makedirs(test_anno, exist_ok=True)
                test_path = os.path.dirname(filename) + "/../test_mouse/JPEGImages/"
                os.makedirs(test_path, exist_ok=True)
                shutil.move(filename, test_path+os.path.split(filename)[1])

                base_path = os.path.dirname(filename) + "/../Annotations"
                if os.path.exists(base_path):
                    label_file = base_path + "/" + os.path.basename(os.path.splitext(filename)[0]) + '.xml'
                else:
                    base_path = os.path.dirname(filename) + "/../xml"
                    if os.path.exists(base_path):
                        label_file = base_path + "/" + os.path.basename(os.path.splitext(filename)[0]) + '.xml'
                    else:
                        label_file = os.path.splitext(filename)[0] + '.xml'
                if os.path.exists(label_file):
                    shutil.move(label_file, test_anno + os.path.split(filename)[1][:-4]+".xml")
            ...
            if filename:
                ...
                # item = None
                self.removeItemWidget(self.takeItem(self.currIndex))
                test_anno = os.path.dirname(filename) + "/../mouse_no/Annotations/"
                os.makedirs(test_anno, exist_ok=True)
                test_path = os.path.dirname(filename) + "/../mouse_no/JPEGImages/"
                os.makedirs(test_path, exist_ok=True)
                shutil.move(filename, test_path + os.path.split(filename)[1])
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None):
        ...
        verify = action('&Verify Image', self.verifyImg,
                        'Ctrl+space', 'verify', u'Verify Image')
        ...
        delete = action('Delete\nRectBox', self.deleteSelectedShape,
                        'Alt+Delete', 'delete', u'Delete', enabled=False)
    ...
    def resetState(self,clear_img=True):
    ...
    def fileSelectionChanged(self):
        ...
        if not self.mayContinue():
            return
        self.currIndex = self.imageList.index(str(item.text()))
        if self.currIndex < len(self.imageList):
            filename = self.imageList[self.currIndex]
            if filename:
                self.loadFile(filename)
    def fileitemDoubleClicked(self, item=None):
        self.currIndex = self.imageList.index(ustr(item.text()))
        if self.currIndex < len(self.imageList):
            filename = self.imageList[self.currIndex]
            if filename:
                self.loadFile(filename)
    ...
    def loadFile(self, filePath=None):
        ...
        if (filePath in self.imageList and
                    self.fileListWidget.currentRow() !=
                    self.imageList.index(filePath)):
            self.fileListWidget.setCurrentRow(self.imageList.index(filePath))
            return
        # unicodeFilePath = ustr(filePath)
        # Tzutalin 20160906 : Add file list and dock to move faster
        # Highlight the file item
        # if filePath and not filePath.endswith('.xml') and self.fileListWidget.count() > 0:
        #     index = self.mImgList.index(filePath)
        #     fileWidgetItem = self.fileListWidget.item(index)
        #     if fileWidgetItem is None:
        #         print("fileWidgetItem is none",index)
        #         return
        ...
        if filePath and os.path.exists(filePath):
            if LabelFile.isLabelFile(filePath):
                try:
                    self.labelFile = LabelFile(filePath)
                except LabelFileError as e:
            ...
            if self.usingPascalVocFormat is True:
                if self.defaultSaveDir is not None:
                    ...
                    if os.path.exists(os.path.join(self.defaultSaveDir,"../Annotations")):
                        xmlPath = os.path.join(self.defaultSaveDir,"../Annotations", basename)
                    elif os.path.exists(os.path.join(self.defaultSaveDir,"../xml")):
                        xmlPath = os.path.join(self.defaultSaveDir,"../xml", basename)
                    else:
                        xmlPath = os.path.join(self.defaultSaveDir, basename)
                    self.loadPascalXMLByFilename(xmlPath)
                else:
                    xmlPath=os.path.dirname(filePath) +"/../Annotations"+"/"+os.path.basename(os.path.splitext(filePath)[0])+ XML_EXT
                    if not os.path.isfile(xmlPath):
                        xmlPath = os.path.dirname(filePath) +"/../xml/"+os.path.basename(os.path.splitext(filePath)[0])+ XML_EXT
                        if not os.path.isfile(xmlPath):
                            xmlPath = os.path.splitext(filePath)[0] + XML_EXT
                    if os.path.isfile(xmlPath):
                        self.loadPascalXMLByFilename(xmlPath)
                    else:
                        self.canvas.loadShapes(None)

            self.setWindowTitle(__appname__ + ' ' + filePath+" "+str(self.currIndex)+"/"+str(len(self.imageList)))
    ...
    def importDirImages(self, dirpath):
        ...
        mImgList = self.scanAllImages(dirpath)

        for imgPath in mImgList:
    ...
    def verifyImg(self, _value=False):
        # Proceding next image without dialog if having any label
         if self.filePath is not None:
            try:
                if self.labelFile is None:
                    print("verifyImg self.labelFile is None")
                    return
    ...
    def openPrevImg(self, _value=False):
        ...
        self.currIndex = self.imageList.index(self.filePath)
        if self.currIndex - 1 >= 0:
            filename = self.imageList[self.currIndex - 1]
            if filename:
                self.loadFile(filename)
    ...
    def openNextImg(self, _value=False):
        ...
        if self.filePath is None:
            ...
            self.currIndex = self.imageList.index(self.filePath)
            if self.currIndex + 1 < len(self.imageList):
                filename = self.imageList[self.currIndex + 1]
    ...
    def saveFile(self, _value=False):
        if self.defaultSaveDir is not None and len(ustr(self.defaultSaveDir)):
            if self.filePath:
                ...
                savedPath = os.path.join(ustr(self.defaultSaveDir),"../Annotations", savedFileName)
                self._saveFile(savedPath)
        else:
            imgFileDir = os.path.dirname(self.filePath)
            imgFileName = os.path.basename(self.filePath)
            savedFileName = os.path.splitext(imgFileName)[0] + XML_EXT
            savedPath = os.path.join(imgFileDir,"../Annotations", savedFileName)
            self._saveFile(savedPath if self.labelFile
                           else self.saveFileDialog(os.path.join(imgFileDir,"../Annotations")))

    def saveFileAs(self, _value=False):
        assert not self.image.isNull(), "cannot save empty image"
        self._saveFile(self.saveFileDialog())

    def saveFileDialog(self,default_path=None):
        caption = '%s - Choose File' % __appname__
        filters = 'File (*%s)' % LabelFile.suffix
        openDialogPath = self.currentPath()
        if default_path is not None:
            openDialogPath=default_path
